...
if [ -f "$DIR/data/keyframes/_failed_videos.txt" ]; then
  echo "[package_results.sh] !!! CẢNH BÁO !!! Vẫn còn video LỖI (data/keyframes/_failed_videos.txt) -" >&2
  echo "  sửa dependency rồi chạy lại đúng video đó TRƯỚC KHI gửi, kẻo thiếu keyframe." >&2
fi
